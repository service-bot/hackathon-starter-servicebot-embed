...
exports.postSignup = (req, res, next) => {
  ...
  const user = new User({
    ...
    let getPlans = async function (service, tier) {
        if (service && tier) {
    ...
    const customer = await stripe.customers.create({email: req.body.email});
      //Get the plan (or plans) for the tier
        ...
        console.log('item obj', {items: items});
    //Create a subscription for that customer
    //use plan from Stripe
        try {
            const subscription = await stripe.subscriptions.create({customer: customer.id, items: items, trial_period_days: 1});
        }
        catch (e){
            console.error(e);
        }
    //Add Stripe Cus ID to the user obj
    user.save((err) => {
      if (err) { return next(err); }
      req.logIn(user, (err) => {
        if (err) {
